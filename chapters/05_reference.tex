...
系统移植是否完成，需要通过可重复的构建流程和可观察的运行结果确认。当前系统需要编译 Rust 内核，交叉编译 C 用户库、Shell 和用户程序，生成包含内核、用户程序和运行资源的磁盘镜像，并通过 QEMU RISC-V virt 机器启动到可交互 Shell。本章说明终端交互环境、构建与运行方式、功能测试范围和结果分析，并在最后总结本文工作与后续完善方向。
...
\begin{table}[htbp]
  \centering
  \caption{运行截图与测试材料安排}\label{tab:screenshot-materials}
  \renewcommand{\arraystretch}{1.25}
  \begin{tabular}{p{0.24\textwidth}p{0.38\textwidth}p{0.26\textwidth}}
    \toprule
    材料 & 运行内容 & 论文中对应位置 \\
    \midrule
    重定向与读取 & 运行 \verb|ls > output|，随后运行 \verb|cat output| & 图 \ref{fig:test-ls-cat-screenshot} \\
    进程创建与等待 & 依次运行 \verb|fork| 和 \verb|forks|，截取 PID 输出和返回 Shell 的结果 & 图 \ref{fig:test-fork-screenshot} \\
    进程间信号 & 运行 \verb|sig|，截取父进程发送信号、子进程处理函数和 wait 输出 & 图 \ref{fig:test-sig-screenshot} \\
    终端控制字符 & 运行 \verb|sigTest| 后在睡眠期间按下 Ctrl-C，截取进程被打断并返回 Shell 的结果 & 图 \ref{fig:test-sigint-screenshot} \\
    \bottomrule
  \end{tabular}
\end{table}
