...
启动与内存管理是操作系统移植中最基础的部分。对于原 IA-32 版本 Unix V6++ 而言，启动过程依赖传统 PC 的实模式、保护模式和引导扇区加载机制，内存分配则采用接近 Unix V6 传统的空闲块表管理方式；而在 RISC-V/QEMU virt 平台上，内核通常由 QEMU 和 OpenSBI 共同引导，进入内核时已经处于 RISC-V 特权架构定义的运行环境中。因此，本课题需要重新设计内核入口、链接布局、早期栈、页表初始化和物理内存管理机制。本章围绕当前代码中的启动流程、Sv39 地址空间设计和页帧分配机制展开说明，并在合适位置与原 IA-32/C++ 版本进行对比。
...
这种差异可以用\figref{fig:mm-lifetime}概括。原版本中，内存块通常以地址和长度的形式在调用链中流转，分配和释放由具体调用点显式完成；重构后，页资源先被分配器创建，再进入拥有所有权的 Rust 对象，最后在对象生命周期结束时通过 \verb|Drop| 自动归还。二者都能表达页资源的申请和释放，但 Rust 版本将“资源属于谁”和“何时释放”放入类型结构中，使后续模块在扩展时更容易检查资源边界。
  ...
  \fbox{%
    \begin{minipage}{0.92\textwidth}
      ...
      \begin{tabular}{p{0.42\textwidth}p{0.42\textwidth}}
        \textbf{原 C++ 管理路径} & \textbf{Rust 重构后管理路径} \\
        \midrule
        \verb|AllocMemory(size)| 返回起始地址 & \verb|KernelPages::alloc(order)| 返回拥有者对象 \\
        调用方保存地址与长度 & 对象内部持有 \verb|PhysPage| 句柄 \\
        调用方在合适位置调用 \verb|FreeMemory| & 生命周期结束时触发 \verb|Drop| \\
        空闲块表插入并合并空闲区间 & Buddy 分配器回收对应页帧 \\
      \end{tabular}
    \end{minipage}
  }
...
需要说明的是，C++ 本身也支持 RAII，但原 Unix V6++ 的内存管理实现主要服务于还原 Unix V6 的管理思想，因此更多采用显式分配、显式释放和全局管理器的组织方式。本文的 Rust 重构并不是否定这种设计，而是在跨平台移植过程中利用 Rust 的所有权、生命周期和 \verb|Drop| 机制，对资源管理边界进行重新表达。这样既保留了教学系统中“页从哪里来、到哪里去”的可观察性，又减少了调用点遗漏释放或释放路径分散带来的维护压力。
