...
从 Rust 重构角度看，本部分的改动并不等于消除所有 \verb|unsafe| 代码。Trap 入口、用户指针读取、信号上下文压栈和系统调用 ABI 都涉及底层内存操作。当前实现的处理方式是把这些边界集中到少数位置，分发、错误转换、信号判断和调度检查则放在类型化的 Rust 函数中。与原版本中多组入口宏和手工寄存器约定相比，这种写法更便于定位一次 Trap 的完整路径。
